...
| Image build | `ephemera build-image --spec examples/build-image.json` — download, verify SHA-256, resize, and customize a base image once, reuse it for every VM. See [Building custom OS images](../images/build-image-tutorial.md) for a per-distro walkthrough (apt/dnf/pacman). |

For full command references, see the [technical docs](/docs/ephemera) and the project's own [README](https://github.com/zyvorai/ephemera#readme). See also: [Use cases](../onboarding/use-cases.md).

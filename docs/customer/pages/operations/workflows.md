...
| Image build | `fluxvm build-image --spec examples/build-image.json` — download, verify SHA-256, resize, and customize a base image once, reuse it for every VM. See [Building custom OS images](../images/build-image-tutorial.md) for a per-distro walkthrough (apt/dnf/pacman). |

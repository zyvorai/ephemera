...
| Image build | `fluxvm build-image --spec examples/build-image.json` — download, verify SHA-256, resize, and customize a base image once, reuse it for every VM. See [Building custom OS images](build-image-tutorial.md) for a per-distro walkthrough (apt/dnf/pacman). |

For full command references, see the [technical docs](/docs/fluxvm) and the project's own [README](https://github.com/zyvorai/fluxvm#readme). See also: [Use cases](use-cases.md).

...
async fn launch_direct(cfg: &Config, req: &CreateVmRequest, ctx: &LaunchContext) -> Result<LaunchResult> {
    ...
    let (program, args) = fluxvm_core::process::netns_wrap(ctx.network.netns.as_deref(), &cfg.firecracker_binary, &args);
...
async fn launch_jailed(cfg: &Config, req: &CreateVmRequest, ctx: &LaunchContext) -> Result<LaunchResult> {
    ...
    let (program, jailer_args) = fluxvm_core::process::netns_wrap(ctx.network.netns.as_deref(), &cfg.jailer.jailer_binary, &jailer_args);
